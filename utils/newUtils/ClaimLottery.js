...
// },});
//
export function ClaimLottery(_lotteryId) {
  const { address, chainId, isConnected } = useWeb3ModalAccount()
  const { walletProvider } = useWeb3ModalProvider()
 async function _ClaimLottery(_lotteryId) {
    try {
    ...
    const tx = await LotteryWithSigner.ClaimLottery(_lotteryId);
    console.log(tx);
    } catch (error) {
  ...
  return <button onClick={ () => _ClaimLottery(_lotteryId)}>Get User Address</button>
} 