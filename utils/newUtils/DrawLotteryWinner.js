...
//
export function DrawLotteryWinner(_lotteryId,
  _randomNumber,
  requestId) {
  const { address, chainId, isConnected } = useWeb3ModalAccount()
  const { walletProvider } = useWeb3ModalProvider()
   async function _DrawLotteryWinner(
    _lotteryId,
    _randomNumber,
    requestId) {
    try {
    ...
    const tx = await LotteryWithSigner.DrawLotteryWinner(_lotteryId,_randomNumber,requestId);
    console.log(tx);
    } catch (error) {
  ...
  return <button onClick={() => _DrawLotteryWinner(
    _lotteryId,
    _randomNumber,
    requestId)}>Get User Address</button>
} 
