...
//
export function CalculatePercentage(_amount,_percentage) {
  const { address, chainId, isConnected } = useWeb3ModalAccount()
  const { walletProvider } = useWeb3ModalProvider()
  async function _calculatePercentage(_amount,_percentage) {
    try {
    ...
    const tx =await LotteryWithSigner.calculatePercentage(_amount,_percentage);
  ...
  return <button onClick={()=>_calculatePercentage(_amount,_percentage)}>Get User Address</button>
}
