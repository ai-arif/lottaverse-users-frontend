...
//
export function GetLotteryStatus(_requestId) {
  const { address, chainId, isConnected } = useWeb3ModalAccount()
  const { walletProvider } = useWeb3ModalProvider()
   async function _getLotteryStatus(_requestId) {
    try {
    ...
    const tx =await LotteryWithSigner.getLotteryStatus(_requestId);
  ...
  return <button onClick={() => _getLotteryStatus(_requestId)}>Get User Address</button>
}