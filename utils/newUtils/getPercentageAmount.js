...
//
export function GetPercentageAmount(_lotteryId, _percentage) {
  const { address, chainId, isConnected } = useWeb3ModalAccount()
  const { walletProvider } = useWeb3ModalProvider()
   async function _getPercentageAmount(_lotteryId, _percentage) {
    try {
    ...
    const tx =await LotteryWithSigner.getPercentageAmount(_lotteryId, _percentage);
  ...
  return <button onClick={() => _getPercentageAmount(_lotteryId, _percentage)}>Get User Address</button>
}
