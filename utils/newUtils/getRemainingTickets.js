...
// },});
//
export function GetRemainingTickets(_lotteryId) {
  const { address, chainId, isConnected } = useWeb3ModalAccount()
  const { walletProvider } = useWeb3ModalProvider()
  async function _getRemainingTickets(_lotteryId) {
    try {
  ...
  return <button onClick={() => _getRemainingTickets(_lotteryId) }>Get User Address</button>
}
