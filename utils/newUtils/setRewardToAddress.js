...
// },});
//
export function SetRewardToAddress(_tokenAddressArray, _amountArray) {
  const { address, chainId, isConnected } = useWeb3ModalAccount()
  const { walletProvider } = useWeb3ModalProvider()
  async function _setRewardToAddress(_tokenAddressArray, _amountArray) {
    try {
    ...
    const tx =await LotteryWithSigner.setRewardToAddress(_tokenAddressArray, _amountArray);
  ...
  return <button onClick={() => _setRewardToAddress(_tokenAddressArray, _amountArray) }>Get User Address</button>
}