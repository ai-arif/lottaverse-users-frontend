...
// },});
//
export function TotalLotteryEarning(_lotteryId) {
  const { address, chainId, isConnected } = useWeb3ModalAccount()
  const { walletProvider } = useWeb3ModalProvider()
 async function _totalLotteryEarning(_lotteryId) {
    try {
    ...
    const tx =await LotteryWithSigner.totalLotteryEarning(_lotteryId);
  ...
  return <button onClick={() => _totalLotteryEarning(_lotteryId) }>Get User Address</button>
}